...
pub enum VcKind {
    ...
    /// Union field reinterpretation cast safety (V130).
    /// Generated when reading a union field to verify the active field matches.
    /// Warning severity.
    UnionAccess,
    /// Drop scope-exit ordering verification (V140).
    /// Verifies that drop order is correct at scope exit.
    /// Warning severity.
    DropOrder,
    /// Pin move-prevention invariant (V150).
...
struct CallSiteInfo {
    ...
    callee_name: String,
    /// Original callee function name before normalization (preserves path qualifiers).
    /// Used for detecting trait implementations like `<IoError as From<ParseError>>::from`.
    raw_callee_name: String,
    ...
    match &block.terminator {
        ...
        Terminator::Call {
            ...
            state.call_sites.push(CallSiteInfo {
                callee_name,
                raw_callee_name: callee_func.clone(),
                args: args.clone(),
...
/// Detect if a raw callee name is a `From` trait implementation call.
///
/// Matches patterns like `<IoError as From<ParseError>>::from` which are
/// generated by the `?` operator in Rust MIR. Returns the contract_db
/// key `"From::from"` if the pattern matches.
///
/// This enables VCGen to look up the `From::from` contract for calls
/// that `normalize_callee_name` reduces to just `"from"`.
fn detect_from_trait_callee(raw_callee: &str) -> Option<String> {
    let trimmed = raw_callee.trim();
    let stripped = trimmed.strip_prefix("const ").unwrap_or(trimmed).trim();
    // Match pattern: <SomeType as From<OtherType>>::from
    if stripped.starts_with('<') && stripped.contains("as From<") && stripped.ends_with(">::from") {
...
/// Detect if a normalized callee name is a known iterator adapter.
///
/// Checks if `"Iterator::{callee_name}"` exists in the contract database.
/// Returns the contract_db key (e.g., `"Iterator::map"`) if found.
fn detect_iterator_adapter_callee(
    callee_name: &str,
    contract_db: &ContractDatabase,
) -> Option<String> {
    let key = format!("Iterator::{}", callee_name);
    if contract_db.get(&key).is_some() {
        Some(key)
    } else {
        None
    }
...
fn generate_call_site_vcs(
    ...
    for path in paths {
        for call_site in &path.call_sites {
            // Try primary lookup, then fallback resolution strategies.
            let resolved_summary = contract_db
                .get(&call_site.callee_name)
                // COMPL-18: From::from trait callee detection (? operator pattern).
                // Raw callee names like "<IoError as From<ParseError>>::from" normalize
                // to just "from", but the contract_db key is "From::from".
                .or_else(|| {
                    detect_from_trait_callee(&call_site.raw_callee_name).and_then(|from_key| {
                        let summary = contract_db.get(&from_key);
                        if summary.is_some() {
                            tracing::debug!(
                                caller = %func.name,
                                raw_callee = %call_site.raw_callee_name,
                                resolved_to = %from_key,
                                "Resolved From::from trait callee at ? operator site"
                            );
                        }
                        summary
                    })
                })
                // COMPL-04: Iterator adapter callee detection.
                // Normalized names like "map", "filter" don't match "Iterator::map" keys.
                .or_else(|| {
                    detect_iterator_adapter_callee(&call_site.callee_name, contract_db).and_then(
                        |iter_key| {
                            let summary = contract_db.get(&iter_key);
                            if summary.is_some() {
                                tracing::debug!(
                                    caller = %func.name,
                                    callee = %call_site.callee_name,
                                    resolved_to = %iter_key,
                                    "Resolved iterator adapter callee"
                                );
                            }
                            summary
                        },
            ...
            let callee_summary = match resolved_summary {
                Some(s) => s,
                None => {
...
fn encode_callee_postcondition_assumptions(
    ...
    // Try primary lookup, then fallback resolution strategies (same as generate_call_site_vcs).
    let callee_summary = match contract_db
        .get(&call_site.callee_name)
        // COMPL-18: From::from trait callee detection (? operator pattern).
        .or_else(|| {
            detect_from_trait_callee(&call_site.raw_callee_name)
                .and_then(|from_key| contract_db.get(&from_key))
        })
        // COMPL-04: Iterator adapter callee detection.
        .or_else(|| {
            detect_iterator_adapter_callee(&call_site.callee_name, contract_db)
                .and_then(|iter_key| contract_db.get(&iter_key))
        }) {
        Some(s) => s,
        None => {
    ...
    // COMPL-18: Special handling for From::from postconditions.
    // The From::from postcondition "result == from_conversion(input)" contains
    // an uninterpreted function `from_conversion` that the spec parser doesn't
    // recognize. We encode it directly as an SMT assertion.
    if detect_from_trait_callee(&call_site.raw_callee_name).is_some() {
        // Get the input argument (first arg to From::from)
        let input_term = if let Some(first_arg) = call_site.args.first() {
            match first_arg {
        ...
        // Assert: result == from_conversion(input)
        // This encodes the From::from postcondition as an SMT uninterpreted function.
        let from_conv_term = Term::App("from_conversion".to_string(), vec![input_term]);
        let postcondition = Term::Eq(Box::new(dest_term), Box::new(from_conv_term));
        tracing::debug!(
            caller = %func.name,
            callee = %call_site.callee_name,
            raw_callee = %call_site.raw_callee_name,
            "Assuming From::from postcondition at ? operator call site"
        );
        script.push(Command::Assert(postcondition));
        return;
    }
