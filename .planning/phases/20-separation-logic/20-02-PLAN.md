...
must_haves:
  ...
  artifacts:
    ...
    - path: "crates/driver/src/callbacks.rs"
      provides: "extract_contracts extended to populate GhostPredicateDatabase from ghost_predicate doc attrs; DB stored as field on AnalysisCallbacks"
  ...
  key_links:
    ...
    - from: "crates/driver/src/callbacks.rs"
      ...
      via: "GhostPredicateDatabase as field on AnalysisCallbacks, read by vcgen during VC generation"
      pattern: "GhostPredicateDatabase"
---
...
Output: `#[ghost_predicate]` macro in macros crate; `ghost_predicate_db.rs` (new); callbacks.rs extended to populate the DB and store it as a field on `AnalysisCallbacks`.
</objective>
...
**Threading strategy: store `GhostPredicateDatabase` as a field named `ghost_pred_db` directly on `AnalysisCallbacks`.**

This is the same pattern used for `ContractDatabase` in callbacks.rs. Follow that exact pattern:

1. Add `ghost_pred_db: GhostPredicateDatabase` as a field on `AnalysisCallbacks`, initialized with `GhostPredicateDatabase::new()` in the struct initializer (the same place `contract_db` is initialized). Do NOT use a different struct or a local-only variable that is discarded — the field must persist on `AnalysisCallbacks` so that Plan 03 can read `self.ghost_pred_db` from the callbacks struct during VC generation.

2. In `extract_contracts()`, use a local `ghost_predicate_db: GhostPredicateDatabase` variable during the doc attribute parsing loop. Add a new branch after the existing `rust_fv::decreases::` branch:
...
3. After the doc attribute loop completes, assign the populated local variable into the struct field: `self.ghost_pred_db = ghost_predicate_db;`. This makes the DB available on the `AnalysisCallbacks` instance for the remainder of the analysis pipeline, including when vcgen.rs reads it in Plan 03.

Do NOT change the return type of `extract_contracts` to avoid call-site ripple effects. The DB is threaded via the struct field, not the return value.
...
- `GhostPredicateDatabase` is stored as field `ghost_pred_db` on `AnalysisCallbacks` so Plan 03 can read `self.ghost_pred_db` during VC generation
