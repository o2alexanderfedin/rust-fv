...
<task type="tdd">
  <name>Task 1: Separation Logic E2E Integration Tests</name>
  <files>tests/sep_logic_integration.rs</files>
  <action>
**Behavior specification — 4 tests, one per SEP requirement:**
...
- Input→Output: `"pts_to(p, v)"` with valid IR → `Some(Term::And([Select(perm,p), Eq(heapval_accessor(Select(sep_heap,p)), v)]))`
...
- Input→Output: `"pts_to(p,x) * pts_to(q,y)"` → `Some(Term::And([pts_to_p, pts_to_q]))`
...
- Input→Output: callee with pts_to in requires → call-site VC script contains `(forall` and `AUFBV`
...
- Input→Output: ghost pred call at depth 3 → expanded body term; at depth 0 → `BoolLit(false)`
...
Create `tests/sep_logic_integration.rs` with all 4 test stubs using `assert!(false, "not yet implemented")` as the body. Run:

```
cargo test --test sep_logic_integration 2>&1
```

All 4 tests MUST fail (RED). Commit: `test(20-04): add failing sep-logic integration tests`.

**GREEN — Implement each test body:**

Implement each test using the helper patterns from existing E2E tests in the codebase:
- Search for `assert_all_unsat`, `Z3Solver::new()`, `Script::render` patterns in vcgen.rs tests.
- Build IR functions using the same pattern as `unsafe_analysis` tests for `RawPtr` parameter types.
- Pass `GhostPredicateDatabase` into the spec parser using `parse_spec_expr_with_db` from Plan 03.
- Render scripts to SMT-LIB string and run Z3 using the existing Z3 integration test helper pattern.
- Look for Z3 E2E patterns in: `crates/analysis/src/vcgen.rs` `#[cfg(test)]` section and `crates/driver/src/` integration tests.

Fix any infrastructure gaps found during test implementation by patching the relevant Plan 01-03 files.
Run tests until all 4 pass. Commit: `feat(20-04): all 4 sep-logic integration tests passing`.

**REFACTOR — Extract shared helpers:**

Extract shared test helpers (e.g., `build_ptr_func(name, ptr_param, val_param, requires_raw)`) to reduce duplication. Ensure each test has a clear comment explaining what SEP requirement it validates. Run tests again to confirm still green. Commit: `refactor(20-04): extract sep-logic test helpers`.
  </action>
  <verify>
